<div class="card">
    ...
    <div class="card-content">
            ...
            <table class="table mb-0 text-center table-lg">
                ...
                <tbody>
                    @forelse ($this->faculties as $faculty)
                        <tr class="text-nowrap">
                            <td class="text-bold-500">{{ $loop->index + $this->faculties->firstItem() }}</td>
                            <td class="text-bold-500">{{ $faculty->name }}</td>
                            <td>
                                <div class="gap-3 d-flex justify-content-center align-items-center">
                                    <button
                                        wire:click="$dispatch('faculty-edit', {faculty_id: '{{ $faculty->id }}'})"
                                        wire:key='{{ $faculty->slug }}' class="btn btn-warning">
                                        <i class="bi bi-pencil-square"></i>
                                    </button>
                                    <button type="button"
                                        wire:click="$dispatch('faculty-delete-confirm', {faculty_id: '{{ $faculty->id }}'})"
        ...
        @if ($this->faculties->total() > $this->faculties->perPage())
            <div class="p-3 pt-4">
                {{ $this->faculties->links() }}
            </div>
        @endif
    </div>
        ...
        <div class="modal-dialog modal-dialog-scrollable" role="document">
            <div class="modal-content">
                ...
                <div class="gap-2 modal-footer d-flex align-items-center">
                    ...
                    <button type="button" class="btn btn-danger ms-1" wire:click="$dispatch('faculty-delete')"
